...
@docs/contribute/contract.md is the Architecture Contract — the numbered Commandments every change under `src/xmris/` must obey, each with its enforcement and live exemplars. Follow it for any library code; where another page differs, the contract wins.

## Documentation style

These four rules govern everything under `docs/` — explanation articles, tutorials, diary entries, and edits to any of them. This section is their single source of truth; the `dev-diary` and `docs-page` skills route here for the rules and restate only the one exception that binds their own genre. *Exception: guides under `docs/contribute/` are exempt from the motivated-narrative rule — a numbered list of commands is the right shape for a setup page.*
...
- Lint + format check, exactly as the `Lint` CI job runs them: `uv run lint` — ruff format, ruff check, then `docs-format` (jupytext round-trip of every paired docs page; `uv run docs-format --fix` repairs drift). `.pre-commit-config.yaml` runs the same three; `uv run pre-commit install` is opt-in. Individually: `uv run ruff check .` (`--fix` to auto-fix), `uv run ruff format .`. Ruff excludes `*.md` — it formats Python inside Markdown code blocks and would flatten the hand-set docs snippets, so `ruff format .` is safe to run tree-wide.
...
- Math/science behavior: MyST `.md` notebooks **are** the tests. `uv run test` converts them to `.ipynb` under `tests/autogen_notebooks/` (gitignored) and runs them via nbmake. `test-gen` walks the five tutorial chapters (`docs/basics/`, `pipeline/`, `fitting/`, `visualization/`, `vendor/`) **and** `docs/concepts/` (explainers that carry a jupytext kernelspec) — each landing under `autogen_notebooks/<chapter>/` — so a live claim in an explainer is executed and asserted like any tutorial cell.
...
- **Fitting is an optional extra.** `pyamares-xmris` (distribution name; import name is still `pyAMARES`) lives in `[project.optional-dependencies].fitting`, not core deps, so a bare `pip install xmris` is fitting-free (and thus free of `hlsvdpro`, which ships no arm64 wheel). `import xmris` never imports pyAMARES eagerly; `xmris.fit_amares` / `da.xmr.fit_amares` resolve it lazily and raise a friendly `ImportError` pointing at `xmris[fitting]` when absent (guards in `src/xmris/__init__.py` + `src/xmris/fitting/__init__.py`; message centralized as `fitting.MISSING_FITTING_DEP_MSG`). `simulate_fid` is pyAMARES-free and stays eager. `pyamares-xmris` is a BSD repackage of pyAMARES **on PyPI** that adds the `hlsvdpro` platform marker (so `pip install xmris[fitting]` installs on Apple Silicon) + the `numpy<2`/`pandas<2.2` caps — so there is no `[tool.uv.sources]` redirect anymore. For local pyAMARES dev, add `[tool.uv.sources] pyamares-xmris = { path = "../pyAMARES", editable = true }`. `uv sync --all-extras --dev` installs it for tests; publish builds use `uv build --no-sources`.
- Jupytext syncs `.md` ↔ `.ipynb`; edit either, but commit ONLY `.md` — `docs/**/*.ipynb` are gitignored. The frozen kernel label is `display_name: Python 3 (xmris)` (jupytext has no `--display-name` flag). To bulk-fix drifted kernels, register the venv kernelspec once with `uv run python -m ipykernel install --sys-prefix --name python3 --display-name "Python 3 (xmris)"`, then `uv run jupytext --set-kernel python3 ./docs/**/*.md`.
- Python is capped at ≤3.13 — a pyAMARES (`fitting` extra) constraint, kept project-wide only because `requires-python` can't be scoped to an optional extra; core xmris has no such cap. The pins `numpy<2` / `pandas<2.2` (in the `fitting` extra — pyAMARES's real limits, which official pyAMARES under-declares), `xarray<2025.11.0`, `griffe<0.40.0`, `pytest-cov<7.0` are deliberate — don't unpin without a reason.
