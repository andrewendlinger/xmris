...
- **Fitting is an optional extra.** `pyamares` lives in `[project.optional-dependencies].fitting`, not core deps, so a bare `pip install xmris` is fitting-free (and thus free of `hlsvdpro`, which ships no arm64 wheel). `import xmris` never imports pyAMARES eagerly; `xmris.fit_amares` / `da.xmr.fit_amares` resolve it lazily and raise a friendly `ImportError` pointing at `xmris[fitting]` when absent (guards in `src/xmris/__init__.py` + `src/xmris/fitting/__init__.py`; message centralized as `fitting.MISSING_FITTING_DEP_MSG`). `simulate_fid` is pyAMARES-free and stays eager. The extra is still git-sourced (`[tool.uv.sources]`, branch `xmris-compatible`) because that branch adds the `hlsvdpro` platform marker + `numpy<2`/`pandas<2.2` caps official pyAMARES lacks — swap the `git` line for the commented `path` line for local pyAMARES dev. `uv sync --all-extras --dev` installs it for tests; publish builds use `uv build --no-sources`.
