...
* **Testing:** `pytest` paired with the `nbmake` plugin (tests are executed inside Jupyter Notebooks).
...
4. **Type Hinting:** Fully type-hint all function signatures. Use `xr.DataArray` consistently so they cross-reference correctly in the docs.
5. **NumPy Docstrings:** Use standard NumPy-style docstrings. These are critical as `quartodoc` parses them for the API reference.
