...
You may notice `pyproject.toml` pins `pyamares` to a git fork and wonder whether the
robustness work belongs upstream instead. It doesn't — and the reason is the same one
that shapes the fix.
