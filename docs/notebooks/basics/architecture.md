...
The solution has two parts: a **Data Dictionary** ([section 3](#data-dictionary)) and a **Decorator Engine** ([section 4](#decorator-engine).
...
```{tip}
Because `ATTRS`, `DIMS`, and `COORDS` are defined as explicit class attributes (not TOML files or plain dicts),
...
class attribute — and the entire package updates automatically.
...
* ``carrier_ppm``: The absolute chemical shift at the center of the RF excitation bandwidth. [ppm]
```
...
flowchart TD
    ...
    B --> C{"'reference_frequency' in .attrs?<br>'carrier_ppm' in .attrs?"}
    ...
    F -- "Yes" --> H["Execute: ppm = carrier + (hz / attrs[ATTRS.reference_frequency])"]
...
1. **Config constants** (`ATTRS.reference_frequency`, `DIMS.frequency`, `COORDS.chemical_shift`) eliminate magic strings everywhere.
