...
`docs/api_reference/` is generated by `uv run docs-api` and gitignored, but its MyST targets are
...
`XmrisPhasingMixin`, `XmrisAccessor`). Never guess: `grep -n "^(xmris" docs/api_reference/*.md`.
