...
|---|---|---|
| The whole design in one sentence | `:::{important}` | `concepts/domains.md` § The goal |
| What you get, at a glance | contract table, input × output | `concepts/domains.md` § What you get |
| What this page calls, at a glance | API-anchor table under the hook | `fitting/quickstart.md`, `fitting/pyamares.md` |
| Tiers that differ in cost | table with a cost column | `pipeline/domain_agnostic_autophase.md` |
| A choice a contributor faces | `{mermaid}` decision tree | `concepts/domains.md` § For contributors |
| One call traced through every layer | `{mermaid}` flowchart TD | `basics/architecture.md` § Putting It All Together |
| Guardrail, one-way door, footgun | `:::{warning}` + verbatim error | `concepts/domains.md` § Guardrails |
...
| [`.xmr.fit_amares()`](#xmris.core.accessor.XmrisAccessor.fit_amares) | fits every voxel |
```

Free functions live under their module (`xmris.<module>.<function>`), accessor methods under the
mixin that defines them (`XmrisProcessingMixin`, `XmrisSpectrumCoordsMixin`, `XmrisFourierMixin`,
`XmrisPhasingMixin`, `XmrisAccessor`). Never guess: `grep -n "^(xmris" docs/api/*.md`.
A missed anchor renders as literal text with no build error, so check the built page.

### Open on the reader's own question

`concepts/vocabulary.md` states the objection as a blockquote and then follows it the whole way
...
- For sophisticated styling (`classDef`, `subgraph`, styled spans) copy the reference diagram in
  `docs/index.md`; for simple decision flowcharts copy `concepts/domains.md`.
